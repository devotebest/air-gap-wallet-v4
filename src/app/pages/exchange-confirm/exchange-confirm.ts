...
export class ExchangeConfirmPage {
  ...
  constructor(
    ...
    const exchangeTotalFee = fromAmount.multipliedBy(
      totalFeeInPercent.dividedBy(100)
    );
    this.fee = exchangeTotalFee.plus(txFee);

    this.fromFiatAmount = new BigNumber(this.exchangeResult.amountExpectedFrom)
      .multipliedBy(this.fromWallet.currentMarketPrice)
      .toNumber();
    this.feeFiatAmount = this.fee
      .multipliedBy(this.fromWallet.currentMarketPrice)
      .toNumber();
    this.toFiatAmount = new BigNumber(this.exchangeResult.amountExpectedTo)
      .multipliedBy(this.toWallet.currentMarketPrice)
      .toNumber();
  }
  ...
  public async prepareTransaction() {
    ...
    const amount = new BigNumber(
      new BigNumber(this.exchangeResult.amountExpectedFrom)
    ).shiftedBy(wallet.coinProtocol.decimals);
    const fee = new BigNumber(this.fee).shiftedBy(
      wallet.coinProtocol.feeDecimals
    );

...
export class AccountTransactionListPage {
  ...
  constructor(
    ...
    if (this.protocolIdentifier === ProtocolSymbols.AE) {
      this.http
        .get(
          `https://api-airgap.gke.papers.tech/api/v1/protocol/ae/migrations/pending/${
  ...
  async init() {
    const supportedActions = this.operationsProvider.getActionsForCoin(
  ...
  getImportAccountAction(): CoinAction {
    return {
      ...
      action: async () => {
        ...
        const ktAddresses: any = await protocol.getAddressesFromPublicKey(
        ...
        if (ktAddresses.length === 0) {
          ...
          for (let [index, _ktAddress] of ktAddresses.entries()) {
            await this.operationsProvider.addKtAddress(
              this.wallet,
              index,
              ktAddresses
            );
  ...
  private getStatusAction(ktAddresses?: string[]): CoinAction {
    return {
      ...
      action: async () => {
        ...
        if (ktAddresses) {
          wallet = await this.operationsProvider.addKtAddress(
            this.wallet,
            0,
            ktAddresses
          );
  ...
  showAeMigrationBanner() {
    return (
      this.walletIsAe() &&
      (this.wallet.currentBalance.gt(0) || this.transactions.length > 0) &&
      this.aeMigratedTokens.gt(0)
    );
  }
  ...
  showAeMigrationScreen() {
    return (
      this.walletIsAe() &&
      (this.wallet.currentBalance.eq(0) && this.transactions.length === 0) &&
      this.aeMigratedTokens.gt(0)
    );
  }
  ...
  openBlockexplorer() {
    ...
    this.openUrl(
      blockexplorer.replace("{{address}}", this.wallet.addresses[0])
    );
  }
  ...
  doRefresh(refresher: any = null) {
    ...
    if (this.protocolIdentifier === ProtocolSymbols.AE) {
      this.http
        .get("https://api-airgap.gke.papers.tech/status")
        .subscribe((result: any) => {
  ...
  async isDelegated(): Promise<void> {
    ...
    const action = isDelegated
      ? this.getStatusAction()
      : this.getDelegateAction();
    this.replaceAction(ActionType.DELEGATE, action);
  }

  async getKtAddresses() {
    const protocol = new TezosKtProtocol();
    const ktAddresses = await protocol.getAddressesFromPublicKey(
      this.wallet.publicKey
    );
    const action =
      ktAddresses.length > 0
        ? this.getStatusAction(ktAddresses)
        : this.getDelegateAction();
    this.replaceAction(ActionType.DELEGATE, action);
    return ktAddresses;
  }

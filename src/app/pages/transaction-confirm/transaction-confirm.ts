...
export class TransactionConfirmPage {
  ...
  async ionViewWillEnter() {
    ...
    this.signedTx = (this.signedTransactionSync
      .payload as SignedTransaction).transaction;
    this.protocol = getProtocolByIdentifier(
      this.signedTransactionSync.protocol
    );
  }
      ...
      .then(txId => {
        ...
        if (this.protocol.identifier === ProtocolSymbols.XTZ) {
          ...
          const ktInterval = setInterval(async () => {
            ...
            const xtzWallets = this.accountProvider
              .getWalletList()
              .filter(
                wallet => wallet.protocolIdentifier === ProtocolSymbols.XTZ
              );
            xtzWallets.forEach(async xtzWallet => {
              const ktAccounts = await ktProtocol.getAddressesFromPublicKey(
                xtzWallet.publicKey
              );
        ...
        // tslint:disable-next-line:no-unnecessary-type-assertion
        const signedTxWrapper = this.signedTransactionSync
          .payload as SignedTransaction;
        const lastTx: {
          protocol: string;
        ...
        this.storageProvider
          .set(SettingsKey.LAST_TX_BROADCAST, lastTx)

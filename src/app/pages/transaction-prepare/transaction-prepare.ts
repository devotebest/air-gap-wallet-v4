...
export class TransactionPreparePage {
  ...
  constructor(
    ...
    this.transactionForm = formBuilder.group({
      address: [
        ...
        Validators.compose([
          Validators.required,
          AddressValidator.validate(this.navParams.get("wallet").coinProtocol)
        ])
      ],
      amount: [
        0,
        Validators.compose([
          Validators.required,
          RegexValidator.validate(/^[0-9]+((\.|,){1}[0-9]*)?$/g)
        ])
      ],
  ...
  useWallet(wallet: AirGapMarketWallet) {
    ...
    this.transactionForm.controls.fee.setValue(
      this.wallet.coinProtocol.feeDefaults.low.toFixed(
        -1 * this.wallet.coinProtocol.feeDefaults.low.e + 1
      )
    );
    ...
    if (this.wallet.protocolIdentifier === "ae") {
      this.http
        .get("https://api-airgap.gke.papers.tech/fees")
        .subscribe((result: any) => {
          if (result && result.low && result.medium && result.high) {
            this.wallet.coinProtocol.feeDefaults.low = new BigNumber(
              result.low
            );
            this.wallet.coinProtocol.feeDefaults.medium = new BigNumber(
              result.medium
            );
            this.wallet.coinProtocol.feeDefaults.high = new BigNumber(
              result.high
            );
            this.transactionForm.controls.fee.setValue(
              this.wallet.coinProtocol.feeDefaults.low.toFixed()
          ...
          this.transactionForm.get("feeLevel").valueChanges.subscribe(val => {
            this._ngZone.run(() => {
              switch (val) {
                ...
                case 1:
                  this.transactionForm.controls.fee.setValue(
                    this.wallet.coinProtocol.feeDefaults.medium.toFixed()
                  );
                  break;
                case 2:
                  this.transactionForm.controls.fee.setValue(
                    this.wallet.coinProtocol.feeDefaults.high.toFixed()
                  );
                  break;
                default:
                  this.transactionForm.controls.fee.setValue(
  ...
  public async prepareTransaction() {
    ...
    const fee = new BigNumber(formFee).shiftedBy(
      this.wallet.coinProtocol.feeDecimals
    );
    ...
    try {
      const {
        airGapTx,
        serializedTx
      } = await this.operationsProvider.prepareTransaction(
        this.wallet,
        formAddress,
        amount,
        fee
  ...
  private setMaxAmount(fee: string) {
    ...
    const amount = this.wallet.currentBalance.shiftedBy(

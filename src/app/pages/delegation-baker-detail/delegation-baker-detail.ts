...
import {
  ...
  BakerConfig
} from "../../services/remote-config/remote-config";
...
export class DelegationBakerDetailPage {
  ...
  async ionViewDidLoad() {
    ...
    const delegationCheck = await this.operationsProvider.checkDelegated(
    ...
    try {
      this.delegationRewards = await kt.delegationRewards(
        this.bakerConfig.address
      );

      // we are already delegating, and to this address
      if (
        delegationCheck.isDelegated &&
        delegationCheck.value === this.bakerConfig.address
      ) {
        const delegatedCycles = this.delegationRewards.filter(value =>
          value.delegatedBalance.isGreaterThan(0)
        );

        this.nextPayout =
          delegatedCycles.length > 0
            ? delegatedCycles[0].payout
            : this.addPayoutDelayToMoment(moment()).toDate();

        // make sure there are at least 7 cycles to wait
        if (
          this.addPayoutDelayToMoment(
            moment(delegationCheck.delegatedDate)
          ).isAfter(this.nextPayout)
        ) {
          this.nextPayout = this.addPayoutDelayToMoment(
            moment(delegationCheck.delegatedDate)
          ).toDate();
        }
      } else {
      ...
      this.avgRoIPerCyclePercentage = this.delegationRewards
        .map(delegationInfo => {
          return delegationInfo.totalRewards
            .plus(delegationInfo.totalFees)
            .div(delegationInfo.stakingBalance);
        })
        .reduce((avg, value) => {
      ...
      this.avgRoIPerCycle = this.avgRoIPerCyclePercentage.multipliedBy(
        this.wallet.currentBalance
      );
    } catch (error) {

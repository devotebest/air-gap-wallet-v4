...
export class ExchangePage {
  ...
  async ionViewWillEnter() {
    if (
      ...
      this.supportedProtocolsFrom = await this.filterValidProtocols(
        supportedProtocolsFrom
      );
      await this.loadWalletsForSelectedProtocol(FROM);

      const supportedProtocolsTo = await this.exchangeProvider.getAvailableToCurrenciesForCurrency(
        this.selectedFromProtocol.identifier
      );
      this.supportedProtocolsTo = await this.filterValidProtocols(
        supportedProtocolsTo,
        false
      );
  ...
  public async filterValidProtocols(
    protocols: string[],
    filterZeroBalance: boolean = true
  ): Promise<string[]> {
  ...
  public async initExchangePage() {
    ...
    await this.protocolSet(
      FROM,
      getProtocolByIdentifier(this.supportedProtocolsFrom[0])
    );
    ...
    const hasShownOnboarding = await this.storageProvider.get(
      SettingsKey.EXCHANGE_INTEGRATION
    );
  ...
  async protocolSet(fromOrTo: string, protocol: ICoinProtocol) {
    ...
    if (fromOrTo === FROM) {
      ...
      if (
        ...
        this.selectedFromProtocol.identifier ===
          this.selectedToProtocol.identifier
      ) {
        if (this.supportedProtocolsTo.length > 0) {
          this.protocolSet(
            TO,
            getProtocolByIdentifier(this.supportedProtocolsTo[0])
          );
        } else {
          return (this.exchangePageState =
            ExchangePageState.NOT_ENOUGH_CURRENCIES);
        }
      }
    }
  ...
  async loadWalletsForSelectedProtocol(fromOrTo: string) {
    if (fromOrTo === FROM) {
        ...
        .filter(
          wallet =>
            wallet.protocolIdentifier ===
              this.selectedFromProtocol.identifier &&
            wallet.currentBalance.isGreaterThan(0)
        );

      // Only set wallet if it's another protocol or not available
      if (
        this.shouldReplaceActiveWallet(
          this.fromWallet,
          this.supportedFromWallets
        )
      ) {
      ...
      this.supportedToWallets = this.accountProvider
        ...
        .filter(
          wallet =>
            wallet.protocolIdentifier === this.selectedToProtocol.identifier
        );
      this.toWallet = this.supportedToWallets[0];
      // Only set wallet if it's another protocol or not available
      if (
        this.shouldReplaceActiveWallet(this.toWallet, this.supportedToWallets)
      ) {
        this.toWallet = this.supportedToWallets[0];
      }
    }
  ...
  private shouldReplaceActiveWallet(
    wallet: AirGapMarketWallet,
    walletArray: AirGapMarketWallet[]
  ): boolean {
    return (
      !wallet ||
      ...
      walletArray.every(
        supportedWallet =>
          !this.accountProvider.isSameWallet(supportedWallet, wallet)
      )
    );
  }
  ...
  async loadDataFromExchange() {
    if (this.fromWallet && this.toWallet) {
      this.minExchangeAmount = new BigNumber(
        await this.exchangeProvider.getMinAmountForCurrency(
  ...
  dismissExchangeOnboarding() {
    ...
    this.storageProvider
      .set(SettingsKey.EXCHANGE_INTEGRATION, true)

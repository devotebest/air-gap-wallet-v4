...
class ChangellyApi {
  ...
  async makeJsonRpcCall<T, R>(method, params: T): Promise<R> {
    ...
    const response = await this.http
      .post<JsonRpcReturnWrapper<R>>(CHANGELLY_BASE_URL, wrapper)
      .toPromise();
    if (response.result) {
  ...
  private convertExchangeIdentifierToAirGapIdentifier(
    identifiers: string[]
  ): string[] {
    return identifiers.map(identifier => {
      return identifierExchangeToAirGapMap.has(identifier)
        ? identifierExchangeToAirGapMap.get(identifier)
        : identifier;
    });
  }

  private convertAirGapIdentifierToExchangeIdentifier(
    identifiers: string[]
  ): string[] {
    return identifiers.map(identifier => {
      return identifierAirGapToExchangeMap.has(identifier)
        ? identifierAirGapToExchangeMap.get(identifier)
        : identifier;
    });
  }
  ...
  getMinAmountForCurrency(
    ...
    fromCurrency = this.convertAirGapIdentifierToExchangeIdentifier([
      fromCurrency
    ])[0];
    toCurrency = this.convertAirGapIdentifierToExchangeIdentifier([
      toCurrency
    ])[0];
  ...
  validateAddress(
    currency: string,
    address: string
  ): Promise<{ result: false; message: string }> {
    ...
    return this.makeJsonRpcCall<Object, { result: false; message: string }>(
      method,
      params
    );
  }
  ...
  createTransaction(
    ...
    return this.makeJsonRpcCall<Object, CreateTransactionResponse>(
      method,
      params
    );
  }
...
class ChangellyExchange extends ChangellyApi implements Exchange {
  ...
  public async getAvailableToCurrenciesForCurrency(
    selectedFrom: string
  ): Promise<string[]> {
...
export class ExchangeProvider implements Exchange {
  ...
  createTransaction(
    ...
    return this.exchange.createTransaction(
      fromCurrency,
      toCurrency,
      address,
      amount

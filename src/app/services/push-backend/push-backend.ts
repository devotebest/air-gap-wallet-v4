...
export class PushBackendProvider {
  ...
  async registerPushMany(pushRequests: PushAddressRequest[]) {
    ...
    return this.http
      .post(
        `${TRANSACTION_BACKEND_URL}api/v1/push_notifications/register/`,
        pushRequests,
        { responseType: "text" }
      )
      .toPromise();
  ...
  async unregisterPush(
    ...
    console.log(
      `PushService: Unregistering wallet ${protocolIdentifier}-${address}`
    );
    const body = {
      address: address,
    ...
    return this.http
      .post(
        `${TRANSACTION_BACKEND_URL}api/v1/push_notifications/unregister/`,
        body,
        { responseType: "text" }
      )
      .toPromise();

...
export class AccountProvider {
  ...
  public activeAccountSubject: ReplaySubject<
    AirGapMarketWallet
  > = new ReplaySubject(1);
  ...
  constructor(
    ...
    this.wallets
      .pipe(
        map(wallets =>
          wallets.filter(wallet => "subProtocolType" in wallet.coinProtocol)
        )
      )
      .subscribe(this.subWallets);
    this.wallets
      .pipe(map(wallets => this.getProtocolsFromWallets(wallets)))
      .subscribe(this.usedProtocols);
  }
  ...
  private async loadWalletsFromStorage() {
    ...
    wallets.forEach(wallet => {
      ...
      if (
        ...
        (airGapWallet.isExtendedPublicKey && airGapWallet.addresses.length < 20)
      ) {
  ...
  public removeWallet(walletToRemove: AirGapMarketWallet): Promise<void> {
    let index = this.walletList.findIndex(wallet =>
      this.isSameWallet(wallet, walletToRemove)
    );
    if (index > -1) {
    ...
    this.pushProvider
      .unregisterWallets([walletToRemove])
      .catch(handleErrorSentry(ErrorCategory.PUSH));
  ...
  public walletByPublicKeyAndProtocolAndAddressIndex(
    ...
    return this.walletList.find(
      wallet =>
        ...
        wallet.protocolIdentifier === protocolIdentifier &&
        wallet.addressIndex === addressIndex
    );
  }

  public walletExists(testWallet: AirGapMarketWallet): boolean {
    return this.walletList.some(wallet =>
      this.isSameWallet(wallet, testWallet)
    );
  }

  public isSameWallet(
    wallet1: AirGapMarketWallet,
    wallet2: AirGapMarketWallet
  ) {
    if (
      !(wallet1 instanceof AirGapMarketWallet) ||
      !(wallet2 instanceof AirGapMarketWallet)
    ) {
      return false;
    }
    return (
